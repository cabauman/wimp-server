...
namespace WIMP_Server.DataServices.Http
{
    public class EsiDataClient : IEsiDataClient
    {
        ...
        {
            var categoryResponse = await _httpClient.GetAsync($"{_esiService}/latest/universe/categories/{SHIP_CATEGORY}");
            ...
            var category = JsonSerializer.Deserialize<EsiReadCategoryDto>(await categoryResponse.Content.ReadAsStringAsync());
            var types = new List<int>();

            foreach (var groupId in category.Groups)
            {
                var groupResponse = await _httpClient.GetAsync($"{_esiService}/latest/universe/groups/{groupId}");
                if (!categoryResponse.IsSuccessStatusCode)
                {
                ...
                var group = JsonSerializer.Deserialize<EsiReadGroupDto>(await groupResponse.Content.ReadAsStringAsync());
            ...
            var namesResponse = await _httpClient.PostAsync($"{_esiService}/latest/universe/names/", requestContent);
            ...
            var ships = JsonSerializer.Deserialize<IEnumerable<EsiNameIdPairDto>>(await namesResponse.Content.ReadAsStringAsync());
        ...
        {
            var systemsResponse = await _httpClient.GetAsync($"{_esiService}/latest/universe/systems/");
            ...
            var allSystemIds = JsonSerializer.Deserialize<IEnumerable<int>>(await systemsResponse.Content.ReadAsStringAsync());
            ...
            while (allSystemIds.Any())
            {
                ...
                var failedIds = fetchSystemsWithIds.Where((_, index) => getSystemTasks.ElementAt(index).Result == null);
                if (failedIds.Any())
                {
        ...
        {
            var systemResponse = await _httpClient.GetAsync($"{_esiService}/latest/universe/systems/{id}/");
            ...
            return JsonSerializer.Deserialize<EsiReadSystemDto>(await systemResponse.Content.ReadAsStringAsync());
        }

        public async Task<EsiReadStargateDto> GetStargateWithId(int id)
        {
            var stargateResponse = await _httpClient.GetAsync($"{_esiService}/latest/universe/stargates/{id}");
            ...
            return JsonSerializer.Deserialize<EsiReadStargateDto>(await stargateResponse.Content.ReadAsStringAsync());
        }
        ...
        public Task<IEnumerable<EsiReadStargateDto>> GetAllStargatesInSystems(IEnumerable<EsiReadSystemDto> systems)
        {
            ...
            while (fetchStargateIds.Any())
            {
                ...
                var failedIds = fetchStargatesWithIds.Where((_, index) => getStargateTasks.ElementAt(index).Result == null);
                if (failedIds.Any())
                {
